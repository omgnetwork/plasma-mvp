...
def test_start_deposit_exit(t, u, root_chain, assert_tx_failed):
    ...
    root_chain.startDepositExit(expected_utxo_pos, value_1, proof)
    ...
    assert_tx_failed(lambda: root_chain.startDepositExit(utxo_pos, value_1, proof))
    # Fails if transaction sender is not the depositor
    assert_tx_failed(lambda: root_chain.startDepositExit(utxo_pos, value_1, proof, sender=t.k1))
    # Fails if utxo_pos is wrong
    assert_tx_failed(lambda: root_chain.startDepositExit(utxo_pos - 1, value_1, proof))
    # Fails if value given is not equal to deposited value
    assert_tx_failed(lambda: root_chain.startDepositExit(utxo_pos, value_1 - 1, proof))
    # Fails if proof given is incorrect
    assert_tx_failed(lambda: root_chain.startDepositExit(utxo_pos, value_1, proof[::-1]))
...
def test_start_exit(t, root_chain, assert_tx_failed):
    ...
    utxo_pos1 = dep_blknum * 1000000000 + 10000 * 0 + 0
...
def test_challenge_exit(t, u, root_chain, assert_tx_failed):
    ...
    sigs = tx1.sig1 + tx1.sig2 + confirmSig1
    root_chain.startDepositExit(utxo_pos1, tx1.amount1, proof, sender=key)
    ...
    root_chain.challengeExit(utxo_pos4, utxo_pos1, tx_bytes4, proof, sigs, confirmSig)
...
def test_finalize_exits(t, u, root_chain):
    ...
    merkle = FixedMerkle(16, [deposit_tx_hash], True)
    proof = merkle.create_membership_proof(deposit_tx_hash)
    utxo_pos1 = dep1_blknum * 1000000000 + 10000 * 0 + 0 * 10000000
    root_chain.startDepositExit(utxo_pos1, tx1.amount1, proof, sender=key)
